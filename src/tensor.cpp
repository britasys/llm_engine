...
DTypeInfo dtype_info(DType dt) {
    ...
    throw std::invalid_argument("dtype_info: unknown DType (" +
...
int64_t checked_mul(int64_t a, int64_t b, const char* what) {
    if (a != 0 && b > std::numeric_limits<int64_t>::max() / a) {
        throw std::overflow_error(std::string(what) + ": integer overflow computing tensor size");
    }
    return a * b;
}
    ...
    for (auto d : shape) {
        if (d <= 0)
            throw std::invalid_argument("Tensor dimensions must be positive, got " +
                                         std::to_string(d));
        n = checked_mul(n, d, "compute_numel");
    }
    return n;
}
...
std::size_t compute_size_bytes(int64_t numel, DType dt) {
    ...
    const auto block_elems = static_cast<int64_t>(info.block_elems);

    if (numel % block_elems != 0)
        throw std::invalid_argument(
            "element count (" + std::to_string(numel) +
            ") is not a multiple of the dtype's block size (" + std::to_string(block_elems) +
            ") for " + dtype_name(dt));

    const int64_t num_blocks = numel / block_elems;
    const int64_t bytes =
        checked_mul(num_blocks, static_cast<int64_t>(info.block_bytes), "compute_size_bytes");
...
Tensor Tensor::view(void* data, std::vector<int64_t> shape, DType dt) {
    if (data == nullptr)
        throw std::invalid_argument("Tensor::view: data pointer must not be null");
...
std::span<float> Tensor::row(int64_t r) {
    ...
    if (r < 0 || r >= shape_[0])
        throw std::out_of_range("row index " + std::to_string(r) + " out of range for " +
                                 shape_string());
